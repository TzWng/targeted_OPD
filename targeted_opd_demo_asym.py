...
class Config:
    ...
    # LOCAL-FIRST loading (2026-07-14): model weights and GSM8K parquets live
    # under Drive's OPD folder (manually downloaded from the HF website),
    # so no HF network call is needed. load_models() copies the model dirs
    # to the VM's local disk once per session (Drive FUSE is slow to read).
    student: str = "/content/drive/MyDrive/OPD/Qwen2.5-0.5B-Instruct"
    teacher: str = "/content/drive/MyDrive/OPD/Qwen2.5-Math-1.5B-Instruct"
    data_dir: str = "/content/drive/MyDrive/OPD"  # holds GSM8K/ parquet folder
    ...
    prompts_per_step: int = 2          # RL-view modes fit 2x8 on 40GB after the
                                       # chunked-logsumexp rework (~23GB peak);
                                       # targeted_kl still wants 1
    group_size: int = 8                # rollouts per question (G) -- papers use 8
    ...
    resume: bool = False               # OFF by default (2026-07-14): resuming
                                       ...
                                       # silently mixed runs. Only turn on to
                                       # continue the SAME variant in the SAME
                                       # (_asym) dir after an interruption.

    # optimization
    lr: float = 1e-5
    steps: int = 300
    kl_coef: float = 0.0               # leash; ref-free => gradient ~0 at current point
    grad_clip: float = 1.0
    seed: int = 0
    ...
    amp_same: float = 1.25             # line-4 coefficient on confirmed-ALIGNED
                                       # tokens (sign(iota)==sign(d_t)): amplify
                                       # the teacher push where the verifier
                                       # endorses it. 1.25 = gopd_lambda, so the
                                       # gopd comparison shares one budget.
    amp_flip: float = 0.25             # coefficient (in |d_t| units) on
                                       # confirmed-CONTRADICTED tokens:
                                       # attenuated reverse step. 0 = abstain,
                                       # 1 = old symmetric flip. DAPO-style
                                       # up/down asymmetry -- reverse updates
                                       # are unanchored, so they get the
                                       # small step.
...
def train_step(cfg, tok, student, teacher, opt, batch_questions, batch_gold):
    ...
    if cfg.mode == "opd":
        ...
        with torch.no_grad(), student.disable_adapter():
            logP_ref = torch.empty(input_ids.shape[0], tgt.shape[1],
                                   device=device, dtype=torch.float32)
            for i in range(0, input_ids.shape[0], 4):
                ...
                for j in range(rl.shape[0]):
                    ...
                    logP_ref[i + j] = lp.gather(-1, tgt[i + j].unsqueeze(-1)).squeeze(-1)
                del rl
        ...
        # coefficients, 2026-07-13): a verdict requires the two INDEPENDENT
        # half-group influence-sign estimates to agree (mass-independent
        # test; an |iota| quantile is confounded with ||grad log pi||). Then
        ...
        # amp_same=1.25 spends gopd's extrapolation budget per-token and
        # verifier-gated instead of blindly; amp_flip<1 is the DAPO-style
        # asymmetry (unanchored reverse updates get the small step).
    ...
    if cfg.kl_coef > 0:
        # k3 estimator of KL(pi_theta || pi_teacher) on sampled tokens
        ratio = (logP_T - logP_S)
...
def _ensure_vllm(cfg):
    ...
    if _VLLM["llm"] is None:
        _VLLM["llm"] = LLM(
            model=cfg.student, enable_lora=True, max_lora_rank=16,
...
def _fresh_adapter_request(cfg, student):
    ...
    return LoRARequest(f"ad{_VLLM['counter']}", _VLLM["counter"], path)

def _vllm_rollouts(cfg, tok, student, prompt_texts):
...
# ----------------------------------------------------------------------------- #
def _ckpt_dir(cfg):
    # "_asym" tag: this file's targeted variant (amp_same/amp_flip line 4)
    # must never share checkpoint dirs with the v2 harness's runs.
    import os
    return os.path.join(cfg.save_dir,
                        f"{cfg.mode}_asym_{cfg.dataset}_seed{cfg.seed}")

def save_ckpt(cfg, student, opt, step, ptr, history):
    # Every save goes to its own step{N}/ subdir -- ALL checkpoints are kept
    # (adapter + optimizer state per snapshot), enabling multi-snapshot
    # report-set evaluation later. No overwriting.
    import os
    try:
...
def try_resume(cfg, student, opt):
    ...
    # Gated by cfg.resume (default False -- see Config note).
    import os
    base = _ckpt_dir(cfg)
...
def run(cfg: Config):
    ...
    opt = torch.optim.AdamW(
        [p for p in student.parameters() if p.requires_grad], lr=cfg.lr)
    ...
    for step in pbar:
        ...
        log = train_step(cfg, tok, student, teacher, opt, qs, gs)
